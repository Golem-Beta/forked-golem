...
【失敗行動的程式碼（重要）】
以下是近期失敗行動對應的原始碼片段。診斷 root_cause 時，請優先從這些程式碼中尋找具體的 bug 或設計缺陷，而非單純依賴 journal 描述。若程式碼顯示問題所在，root_cause 應引用具體的函式名稱或邏輯：
...
【過去失敗的 patch 記錄（最近 8 筆）】
以下是過去被 drop 的提案，包含原因。診斷時必須明確說明本次目標為何不同於這些失敗路徑，或為何相同路徑這次有新根據。

{{PATCH_HISTORY}}

注意：若你的診斷指向與上方相同的 target，必須在 evidence 中明確說明「新根據是什麼」，不能只重複過去的失敗方向。
