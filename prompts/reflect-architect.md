...
{{RETRY_FEEDBACK}}【過去失敗的 patch 記錄（必讀）】
以下是過去被 drop 的提案。target_node 必須選擇與這些失敗目標不同的節點，或能提供新根據說明為何相同目標這次可行：
...
用 JSON 回覆（只輸出 JSON）：
{
  ...
  "risks": ["這個修改的潛在風險"],
  "rationale": "選擇此 target_node 的根據（必填）：說明為何這個節點是根本問題所在，且與過去失敗記錄有何不同",
...
- rationale 必填，且必須說明為何此次與過去失敗的選擇不同或有新根據
- 只輸出 JSON
